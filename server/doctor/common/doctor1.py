...
class Doctor1:
    def __init__(self, config_params):
        ...
        self.peers = ["doctor"+str(i) for i in range(1,self.peers_amount+1)][::-1]
    ...
    def handle_election_message(self, message, addr):
        ...
        if sender_id < self.process_id:
            self.messageHandler.send_message(addr, MessageType.OK, self.process_id)
            if self.get_in_election():
                self.start_election()
    ...
    def handler_ok_message(self, message):
        ...
        sender_id = message["id"]
        print(f"[OK] Received OK from {sender_id}")
    ...
    def is_leader_alive(self):
        return 30>(time.time() - self.last_heartbeat)  
